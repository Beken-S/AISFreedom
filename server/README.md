...
Для работы с API необходимо установить Docker и Docker Compose.

### Команды запуска

```sh
 npm run api:build  # создание docker образов.
 npm run api:up # запуск docker контейнеров.
 npm run api:down # остановка docker контейнеров.
 npm run api-dev:up # запуск docker контейнеров в development режиме.
 npm run api-dev:down # остановка docker контейнеров запущенных в development режиме.
```
